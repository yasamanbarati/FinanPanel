import {
  profileMenuSettingType,
  MenuItemType,
  ContractListType,
  ContractManagementProps,
  UserListProps,
  UserContractProps,
  userTransactionsType,
} from './type';
import {
  ManageAccountIcon,
  PasswordIcon,
  LogOut2Icon,
} from '@/components/icons';
export const profileMenuSetting: profileMenuSettingType[] = [
  {
    title: 'Manage Account',
    icon: ManageAccountIcon,
  },
  {
    title: 'Change Password',
    icon: PasswordIcon,
  },
  {
    title: 'Log out',
    icon: LogOut2Icon,
  },
];

export const SidebarItems: MenuItemType[] = [
  {
    id: 0,
    icon: 'Overviwe',
    activeIcon: 'Overviwe2',
    title: 'Overview',
    path: '/',
  },
  {
    id: 1,
    icon: 'Contract',
    activeIcon: 'Contract2',
    title: 'Contracts',
    path: '/contracts',
    subMenu: [
      {
        path: '/contracts/list',
        title: 'List',
      },
      {
        path: '/contracts/management',
        title: 'Management',
      },
      {
        path: '/contracts/history',
        title: 'History',
      },
    ],
  },

  {
    id: 2,
    icon: 'Users',
    activeIcon: 'Users2',
    title: 'Users List',
    path: '/users',
  },
  {
    id: 3,
    icon: 'Transactions',
    activeIcon: 'Transactions2',
    title: 'Transactions',
    path: '/transactions',
  },
  {
    id: 4,
    icon: 'Webpage',
    activeIcon: 'Webpage',
    title: 'Static Pages',
    path: '/pages',
  },
];

export const HEADER_ITEMS = [
  {
    title: 'New Income',
    profit: '$3,850',
    percent: '8.5%',
    icon: '/static/icons/card1.svg',
  },
  {
    title: 'Total Income',
    profit: '$40,689',
    percent: '16.2%',
    icon: '/static/icons/card2.svg',
  },
  {
    title: 'New Contracts',
    profit: '28',
    percent: '24%',
    icon: '/static/icons/card3.svg',
  },
  {
    title: 'All Contracts',
    profit: '28',
    percent: '24%',
    icon: '/static/icons/card4.svg',
  },
];

export const USERS: UserListProps[] = [
  {
    id: '#345896',
    fullName: 'Sofia Miller',
    email: 'Sofia Miller@gmail.com',
    contractCount: '12',
    status: 1,
    action: 'Deposit',
    avatar: '/static/images/user-1.svg',
    date: 'July 3, 2024',
    walletBalance: '$45.50',
  },
  {
    id: '#321654',
    fullName: 'John Manson',
    email: 'Ashley Williams@gmail.com',
    contractCount: '4',
    status: 0,
    action: 'Deposit',
    avatar: '/static/images/user-2.svg',
    date: 'July 3, 2024',
    walletBalance: '$45.50',
  },
  {
    id: '#345896',
    fullName: 'John Manson',
    email: 'ohnManson@gmail.com',
    contractCount: '8',
    status: 0,
    action: 'Deposit',
    avatar: '/static/images/user-3.svg',
    date: 'July 3, 2024',
    walletBalance: '$45.50',
  },
  {
    id: '#345896',
    fullName: 'John Manson',
    email: 'Millersoha10@gmail.com',
    contractCount: '12',
    status: 1,
    action: 'Deposit',
    avatar: '/static/images/user-4.svg',
    date: 'July 3, 2024',
    walletBalance: '$45.50',
  },
  {
    id: '#345896',
    fullName: 'John Manson',
    email: 'Millersoha10@gmail.com',
    contractCount: '12',
    status: 1,
    action: 'Deposit',
    avatar: '/static/images/user-5.svg',
    date: 'July 3, 2024',
    walletBalance: '$45.50',
  },
];

export const ContractList: ContractListType[] = [
  {
    id: 0,
    status: 1, //active
    ImageSrc: '/static/images/Group.png',
    title: 'Lutetium',
    time: 6,
    minimumAmount: 500.0,
    monthlyProfit: 25,
    dropDown: 5,
    risk: 12,
    limitContrcts: 150,
    activeContracts: 98,
    ListFeatures: [
      {
        id: 0,
        name: 'Up to 15% interest rates',
        flag: true,
      },
      {
        id: 1,
        name: 'Low risk',
        flag: true,
      },
      {
        id: 2,
        name: 'Monthly Profit payment',
        flag: true,
      },
      {
        id: 3,
        name: 'Access to monthly reports',
        flag: true,
      },
    ],
    description:'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna. ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna.',
  },
  {
    id: 1,
    status: 1, //active
    ImageSrc: '/static/images/Group (1).png',
    title: 'Titanium',
    time: 9,
    minimumAmount: 500.0,
    monthlyProfit: 25,
    dropDown: 5,
    risk: 12,
    limitContrcts: 150,
    activeContracts: 98,
    ListFeatures: [
      {
        id: 0,
        name: 'Up to 15% interest rates',
        flag: true,
      },
      {
        id: 1,
        name: 'Low risk',
        flag: true,
      },
      {
        id: 2,
        name: 'Monthly Profit payment',
        flag: true,
      },
      {
        id: 3,
        name: 'Access to monthly reports',
        flag: true,
      },
    ],
    description:
      'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna. ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna.',
  },
  {
    id: 2,
    status: 1, //active
    ImageSrc: '/static/images/Group (2).png',
    title: 'Platinum',
    time: 12,
    minimumAmount: 500.0,
    monthlyProfit: 25,
    dropDown: 5,
    risk: 12,
    limitContrcts: 150,
    activeContracts: 98,
    ListFeatures: [
      {
        id: 0,
        name: 'Up to 15% interest rates',
        flag: true,
      },
      {
        id: 1,
        name: 'Low risk',
        flag: true,
      },
      {
        id: 2,
        name: 'Monthly Profit payment',
        flag: true,
      },
      {
        id: 3,
        name: 'Access to monthly reports',
        flag: true,
      },
    ],
    description:
      'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna. ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna.',
  },
  {
    id: 3,
    status: 0, //Deactive
    ImageSrc: '/static/images/Group (3).png',
    title: 'Lutetium',
    time: 6,
    minimumAmount: 500.0,
    monthlyProfit: 25,
    dropDown: 5,
    risk: 12,
    limitContrcts: 150,
    activeContracts: 98,
    ListFeatures: [
      {
        id: 0,
        name: 'Up to 15% interest rates',
        flag: true,
      },
      {
        id: 1,
        name: 'Low risk',
        flag: true,
      },
      {
        id: 2,
        name: 'Monthly Profit payment',
        flag: true,
      },
      {
        id: 3,
        name: 'Access to monthly reports',
        flag: true,
      },
    ],
    description:
      'Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna. ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna.',
  },
];

export const ContractManagement: ContractManagementProps[] = [
  {
    title: 'Wallets',
    value: 5502.45,
    percent: 12.5,
    status: 1,
  },
  {
    title: 'Contracts',
    value: 945000,
    percent: 27,
    status: 1,
  },
  {
    title: 'Total',
    value: 1594555,
    percent: -15,
    status: 0,
  },
];

export const userList: UserListProps[] = [
  {
    id: '#345896',
    fullName: 'Yasaman Barati',
    email: 'yasamanbarati12@gmail.com',
    walletBalance: '$27,000',
    registerDate: '10/09/2024',
    lastLogin: '18/12/2024 - 18:58:37',
    status: 0,
    avatar: '/static/images/user-2.svg',
  },
  {
    id: '#345897',
    fullName: 'Sofia Miller',
    email: 'sofiamiller12@gmail.com',
    walletBalance: '$15,000',
    registerDate: '10/10/2024',
    lastLogin: '18/12/2024 - 20:58:37',
    status: 1,
    avatar: '/static/images/user-4.svg',
  },
  {
    id: '#345891',
    fullName: 'Sofia Miller',
    email: 'sofiamiller12@gmail.com',
    walletBalance: '$15,000',
    registerDate: '10/10/2024',
    lastLogin: '18/12/2024 - 20:58:37',
    status: 1,
    avatar: '/static/images/user-4.svg',
  },
  {
    id: '#34589',
    fullName: 'Sofia Miller',
    email: 'sofiamiller12@gmail.com',
    walletBalance: '$15,000',
    registerDate: '10/10/2024',
    lastLogin: '18/12/2024 - 20:58:37',
    status: 0,
    avatar: '/static/images/user-4.svg',
  },
  {
    id: '#347896',
    fullName: 'Sofia Miller',
    email: 'sofiamiller12@gmail.com',
    walletBalance: '$15,000',
    registerDate: '10/10/2024',
    lastLogin: '18/12/2024 - 20:58:37',
    status: 0,
    avatar: '/static/images/user-4.svg',
  },
  {
    id: '#346897',
    fullName: 'Sofia Miller',
    email: 'sofiamiller12@gmail.com',
    walletBalance: '$15,000',
    registerDate: '10/10/2024',
    lastLogin: '18/12/2024 - 20:58:37',
    status: 1,
    avatar: '/static/images/user-4.svg',
  },
  {
    id: '#344891',
    fullName: 'Sofia Miller',
    email: 'sofiamiller12@gmail.com',
    walletBalance: '$15,000',
    registerDate: '10/10/2024',
    lastLogin: '18/12/2024 - 20:58:37',
    status: 1,
    avatar: '/static/images/user-4.svg',
  },
  {
    id: '#345189',
    fullName: 'Sofia Miller',
    email: 'sofiamiller12@gmail.com',
    walletBalance: '$15,000',
    registerDate: '10/10/2024',
    lastLogin: '18/12/2024 - 20:58:37',
    status: 0,
    avatar: '/static/images/user-4.svg',
  },
  {
    id: '#315896',
    fullName: 'Sofia Miller',
    email: 'sofiamiller12@gmail.com',
    walletBalance: '$15,000',
    registerDate: '10/10/2024',
    lastLogin: '18/12/2024 - 20:58:37',
    status: 0,
    avatar: '/static/images/user-4.svg',
  },
  {
    id: '#325897',
    fullName: 'Sofia Miller',
    email: 'sofiamiller12@gmail.com',
    walletBalance: '$15,000',
    registerDate: '10/10/2024',
    lastLogin: '18/12/2024 - 20:58:37',
    status: 1,
    avatar: '/static/images/user-4.svg',
  },
  {
    id: '#335891',
    fullName: 'Sofia Miller',
    email: 'sofiamiller12@gmail.com',
    walletBalance: '$15,000',
    registerDate: '10/10/2024',
    lastLogin: '18/12/2024 - 20:58:37',
    status: 1,
    avatar: '/static/images/user-4.svg',
  },
  {
    id: '#349589',
    fullName: 'Sofia Miller',
    email: 'sofiamiller12@gmail.com',
    walletBalance: '$15,000',
    registerDate: '10/10/2024',
    lastLogin: '18/12/2024 - 20:58:37',
    status: 0,
    avatar: '/static/images/user-4.svg',
  },
];

export const transactionsList: UserListProps[] = [
  {
    id: '#345891',
    fullName: 'Sofia Miller',
    type: 'Deposit',
    amount: '$1000',
    date: '18/12/2024 - 20:58:37',
    walletAddress: '1A1zP1eP5QGefi2DM4jaertn',
    status: 0,
    avatar: '/static/images/user-4.svg',
  },
  {
    id: '#345892',
    fullName: 'Sofia Miller',
    type: 'Transfer',
    amount: '$50',
    date: '18/12/2024 - 20:58:37',
    walletAddress: '1A1zP1eP5QGefi2DM4jaertn',
    status: 1,
    avatar: '/static/images/user-4.svg',
  },
  {
    id: '#345893',
    fullName: 'Sofia Miller',
    type: 'Investment',
    amount: '$600',
    date: '18/12/2024 - 20:58:37',
    walletAddress: '1A1zP1eP5QGefi2DM4jaertn',
    status: 2,
    avatar: '/static/images/user-4.svg',
  },
  {
    id: '#345894',
    fullName: 'Sofia Miller',
    type: 'Withdraw',
    amount: '$1000',
    date: '18/12/2024 - 20:58:37',
    walletAddress: '1A1zP1eP5QGefi2DM4jaertn',
    status: 2,
    avatar: '/static/images/user-4.svg',
  },
  {
    id: '#345895',
    fullName: 'Sofia Miller',
    type: 'Deposit',
    amount: '$1000',
    date: '18/12/2024 - 20:58:37',
    walletAddress: '1A1zP1eP5QGefi2DM4jaertn',
    status: 0,
    avatar: '/static/images/user-4.svg',
  },
  {
    id: '#345896',
    fullName: 'Sofia Miller',
    type: 'Transfer',
    amount: '$50',
    date: '18/12/2024 - 20:58:37',
    walletAddress: '1A1zP1eP5QGefi2DM4jaertn',
    status: 1,
    avatar: '/static/images/user-4.svg',
  },
  {
    id: '#345897',
    fullName: 'Sofia Miller',
    type: 'Investment',
    amount: '$600',
    date: '18/12/2024 - 20:58:37',
    walletAddress: '1A1zP1eP5QGefi2DM4jaertn',
    status: 2,
    avatar: '/static/images/user-4.svg',
  },
  {
    id: '#345898',
    fullName: 'Sofia Miller',
    type: 'Withdraw',
    amount: '$1000',
    date: '18/12/2024 - 20:58:37',
    walletAddress: '1A1zP1eP5QGefi2DM4jaertn',
    status: 2,
    avatar: '/static/images/user-4.svg',
  },
  {
    id: '#345899',
    fullName: 'Sofia Miller',
    type: 'Deposit',
    amount: '$1000',
    date: '18/12/2024 - 20:58:37',
    walletAddress: '1A1zP1eP5QGefi2DM4jaertn',
    status: 0,
    avatar: '/static/images/user-4.svg',
  },
  {
    id: '#345812',
    fullName: 'Sofia Miller',
    type: 'Transfer',
    amount: '$50',
    date: '18/12/2024 - 20:58:37',
    walletAddress: '1A1zP1eP5QGefi2DM4jaertn',
    status: 1,
    avatar: '/static/images/user-4.svg',
  },
  {
    id: '#345823',
    fullName: 'Sofia Miller',
    type: 'Investment',
    amount: '$600',
    date: '18/12/2024 - 20:58:37',
    walletAddress: '1A1zP1eP5QGefi2DM4jaertn',
    status: 2,
    avatar: '/static/images/user-4.svg',
  },
  {
    id: '#345834',
    fullName: 'Sofia Miller',
    type: 'Withdraw',
    amount: '$1000',
    date: '18/12/2024 - 20:58:37',
    walletAddress: '1A1zP1eP5QGefi2DM4jaertn',
    status: 2,
    avatar: '/static/images/user-4.svg',
  },
  {
    id: '#345841',
    fullName: 'Sofia Miller',
    type: 'Deposit',
    amount: '$1000',
    date: '18/12/2024 - 20:58:37',
    walletAddress: '1A1zP1eP5QGefi2DM4jaertn',
    status: 0,
    avatar: '/static/images/user-4.svg',
  },
  {
    id: '#345852',
    fullName: 'Sofia Miller',
    type: 'Transfer',
    amount: '$50',
    date: '18/12/2024 - 20:58:37',
    walletAddress: '1A1zP1eP5QGefi2DM4jaertn',
    status: 1,
    avatar: '/static/images/user-4.svg',
  },
  {
    id: '#345863',
    fullName: 'Sofia Miller',
    type: 'Investment',
    amount: '$600',
    date: '18/12/2024 - 20:58:37',
    walletAddress: '1A1zP1eP5QGefi2DM4jaertn',
    status: 2,
    avatar: '/static/images/user-4.svg',
  },
  {
    id: '#345874',
    fullName: 'Sofia Miller',
    type: 'Withdraw',
    amount: '$1000',
    date: '18/12/2024 - 20:58:37',
    walletAddress: '1A1zP1eP5QGefi2DM4jaertn',
    status: 2,
    avatar: '/static/images/user-4.svg',
  },
];
export const userContractsList: UserContractProps[] = [
  {
    id: 0,
    invoice: '#100958',
    status: 0, //In Progress
    imageSrc: '/static/images/Group.png',
    title: 'Lutetium',
    time: 6,
    amount: 500.0,
    profit: 25,
    profitAmount: 80,
    startDate: '10/10/2024',
    endDate: '10/10/2025',
  },
  {
    id: 1,
    invoice: '#100958',
    status: 1, //Withdraw
    imageSrc: '/static/images/Group (2).png',
    title: 'Lutetium',
    time: 6,
    amount: 500.0,
    profit: 25,
    profitAmount: 80,
    startDate: '10/10/2024',
    endDate: '10/10/2025',
  },
  {
    id: 2,
    invoice: '#100958',
    status: 2, //Expired
    imageSrc: '/static/images/Group (1).png',
    title: 'Lutetium',
    time: 6,
    amount: 500.0,
    profit: 25,
    profitAmount: 80,
    startDate: '10/10/2024',
    endDate: '10/10/2025',
  },
];
export const userTransactions: userTransactionsType = {
  userId: '#LM24859',
  withdraw: 45.5,
  makeContract: 500,
  deposit: 200,
  transactions: [
    {
      date: 'July 3, 2024',
      type: 'Transfer',
      user: 'Emily Davis',
      code: '#LM100253',
      amount: '$45.50',
    },
    {
      date: 'July 3, 2024',
      type: 'Transfer',
      user: 'David Santil',
      code: '#SB569851',
      amount: '$20.00',
    },
    {
      date: 'July 1, 2024',
      type: 'Withdraw',
      user: 'David Santil',
      code: '#SB569851',
      amount: '$45.50',
    },
    {
      date: 'July 1, 2024',
      type: 'Make Contract',
      user: 'David Santil',
      code: '#SB569851',
      amount: '$500.00',
    },
    {
      date: 'July 1, 2024',
      type: 'Deposit',
      user: 'David Santil',
      code: '#SB569851',
      amount: '$200.00',
    },
  ],
};